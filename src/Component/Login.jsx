    ...
    async function postData(e) {
        ...
        if (Object.values(errorMessage).find((x) => x && x.length !== 0)) {
            ...
            if (item) {
                ...
                if (item.role === "Admin") {
                    navigate("/admin");
                    // Refresh the page only if the role is admin
                    window.location.reload(); //isse kya hoga ki jo humne app.jsx mai admin route pr validation di hai uske liye ek baar relodad krna padega
                } else {
                    navigate("/");
                }
