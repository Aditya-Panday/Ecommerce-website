    ...
    <div className='container-fluid my-2'>
      <div className='row'>
        ...
        <div className='col-md-9'>
          <h4 className='bg-primary p-2 text-light text-center '> Brand <Link to="/admin/brands/create"><i className='fa fa-plus float-end text-dark '></i></Link> </h4>
            ...
            <tbody>
              {
                currentItems.map((item, index) => {
                  return <tr key={index}>
                    <td>{item.id}</td>
                    <td>{item.name}</td>
                    <td><Link className="btn btn-primary" to={`/admin/brands/update/${item.id}`}><i className='fa fa-edit'></i></Link></td>
